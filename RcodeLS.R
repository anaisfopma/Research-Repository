...
LS.function <- function(n.sim, time.sim, model, p, tau, tf, n.boot){
  ...
  for(x in 1:n.sim){
    ...
    theta.res<-function(residual){
      ...
      estim<-nlminb(as.numeric(log(prm.LS)),cost.res)
      return(estim$par)
    }
    ...
    out.res<-ode(y=c(L=0), times=time.sim, func = model1, parms =c(p=prm.LS[1],tau=tau), rtol = 1e-12,method = "lsode")
    res<- data.sim.noise-out.res[,2] # compute the residuals
    
    boot<-bootstrap(res,n.boot,theta.res)   # here we use n.boot boostrap with the residual res and the function theta.res to estimate all boostrap step
    resu<-exp(boot$thetastar)  # this has the estimates for each boostrap step
  ...
  # MSE
  MSE <- sum(estimates - p)^2 * 1/n.sim  
  print(MSE)
  
  # RSE
  SE <- sd(estimates)/sqrt(length(estimates)) # first obtain the standard error
  RSE <- SE/prm.LS # then calculate the RSE
  print(RSE)
...
# No transformation
set.seed(987)
LS.function(n.sim = 100, time.sim = time.sim, model = model1, p = p, tau = tau, tf = "none", n.boot = 200)

# Arcsine square root transformation
set.seed(987)
LS.function(n.sim = 100, time.sim = time.sim, model = model1, p = p, tau = tau, tf = "arcsinesq", n.boot = 200)

# Square root transformation
set.seed(987)
LS.function(n.sim = 10, time.sim = time.sim, model = model1, p = p, tau = tau, tf = "sqroot", n.boot = 20)

# Log transformation
set.seed(987)
LS.function(n.sim = 100, time.sim = time.sim, model = model1, p = p, tau = tau, tf = "log", n.boot = 200)

# Quadratic root transformation
set.seed(987)
LS.function(n.sim = 100, time.sim = time.sim, model = model1, p = p, tau = tau, tf = "qdroot", n.boot = 200)

# Arcsine quadratic root transformation
set.seed(987)
LS.function(n.sim = 100, time.sim = time.sim, model = model1, p = p, tau = tau, tf = "arcsineqd", n.boot = 200)
...
# Square root transformation
set.seed(987)
LS.function(n.sim = 100, time.sim = time.sim, model = model1, p = p, tau = tau, tf = "sqroot", n.boot = 200)
